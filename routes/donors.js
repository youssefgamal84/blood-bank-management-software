...
router.post("/add", authNurse, checkDonor, (req, res) => {
    ...
    db.addDonor(donor, callback(req, res));
});

router.get("/:ssn", authNurse, (req, res) => {
    var ssn = req.params.ssn;
    db.getDonorBySSN(ssn, callback(req, res, "donorInfo"));
});

router.patch("/:ssn", authNurse, checkDonor, (req, res) => {
    var donor = req.donor;

    db.updateDonorBySSN(req.params.ssn, donor, callback(req, res));
});

router.get("/", authNurse, (req, res) => {
    db.getAllDonors(callback(req, res, "donors"));
});
